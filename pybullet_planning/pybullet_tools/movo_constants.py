...
MOVO_INFOS = {
    arm: IKFastInfo(
        module_name="movo.movo_{}_arm_ik".format(arm),
        base_link="base_link",
        ee_link=EE_LINK.format(arm),
        free_joints=["linear_joint", "{}_arm_half_joint".format(arm)],
    )
    for arm in ARMS
}
...
def has_kg3_gripper(robot, arm):
    assert arm in ARMS
    return all(
        has_joint(robot, joint_name)
        for joint_name in names_from_templates(KG3_GRIPPER_JOINTS, arm)
    )
...
def has_robotiq_gripper(robot, arm):
    ...
    return all(
        has_joint(robot, joint_name)
        for joint_name in names_from_templates(ROBOTIQ_GRIPPER_JOINTS, arm)
    )
...
def get_colliding(robot):
    ...
    for link1, link2 in combinations(get_all_links(robot), r=2):
        if not are_links_adjacent(robot, link1, link2) and pairwise_link_collision(
...
NEVER_COLLISIONS = [
    ("linear_actuator_fixed_link", "right_base_link"),
    ("linear_actuator_fixed_link", "right_shoulder_link"),
    ("linear_actuator_fixed_link", "left_base_link"),
    ("linear_actuator_fixed_link", "left_shoulder_link"),
    ("linear_actuator_fixed_link", "front_laser_link"),
    ("linear_actuator_fixed_link", "rear_laser_link"),
    ("linear_actuator_link", "pan_link"),
    ("linear_actuator_link", "right_shoulder_link"),
    ("linear_actuator_link", "right_arm_half_1_link"),
    ("linear_actuator_link", "left_shoulder_link"),
    ("linear_actuator_link", "left_arm_half_1_link"),
    ("right_wrist_spherical_2_link", "right_robotiq_coupler_link"),
    ("right_wrist_3_link", "right_robotiq_coupler_link"),
    ("right_wrist_3_link", "right_gripper_base_link"),
    ...
    ("left_wrist_spherical_2_link", "left_gripper_base_link"),
    ("left_wrist_3_link", "left_gripper_base_link"),
]
