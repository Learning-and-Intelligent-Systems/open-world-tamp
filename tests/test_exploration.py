...
def test_run_planner_astar_movo():
    parser = create_parser()
    args = parser.parse_args(["--robot", "movo", "--goal", "none", "--world", "empty_room", "--exploration", "--base-planner", "astar"])
    main(args)

def test_run_planner_astar_spot():
    parser = create_parser()
    args = parser.parse_args(["--robot", "spot", "--goal", "none", "--world", "empty_room", "--exploration", "--base-planner", "astar"])
    main(args)