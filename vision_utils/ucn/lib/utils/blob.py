...
def im_list_to_blob(ims, num_channels):
    ...
    num_images = len(ims)
    blob = np.zeros(
        (num_images, max_shape[0], max_shape[1], num_channels), dtype=np.float32
    )
...
def pad_im(im, factor, value=0):
    ...
    if len(im.shape) == 3:
        ...
        return np.lib.pad(
            im, ((0, pad_height), (0, pad_width)), "constant", constant_values=value
        )
...
def chromatic_transform(im, label=None, d_h=None, d_s=None, d_l=None):
    """Given an image array, add the hue, saturation and luminosity to the
    image."""
...
def add_noise_cuda(image, level=0.1):
    ...
    if r < 0.8:
        ...
        motion_blur_filter = nn.Conv2d(
            in_channels=image.size(2),
