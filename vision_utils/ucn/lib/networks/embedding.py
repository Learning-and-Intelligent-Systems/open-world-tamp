...
def compute_cluster_mean(x, cluster_masks, K, normalize):
    """Computes the spherical mean of a set of unit vectors. This is a PyTorch
    implementation The definition of spherical mean is minimizes cosine
    similarity to a set of points instead of squared error.
    ...
    for k in range(K):
        ...
        cluster_means[:, :, k] = torch.sum(x * mask, dim=[2, 3]) / (
            torch.sum(mask, dim=[2, 3]) + 1e-10
        )

    # normalize to compute spherical mean
    if normalize:
        cluster_means = F.normalize(
            cluster_means, p=2, dim=1
        )  # Note, if any vector is zeros, F.normalize will return the zero vector
...
class EmbeddingLoss(nn.Module):
    ...
    def forward(self, x, cluster_masks):
        """Compute the clustering loss. Assumes the batch is a sequence of
        consecutive frames.
        ...
        cluster_means = compute_cluster_mean(
            x, cluster_masks, K, self.normalize
        ...
        tiled_cluster_means = torch.zeros_like(
            x, device=x.device
        )  # Shape: [batch_size x C x H x W]
        for k in range(K):
            mask = (cluster_masks == k).float()  # Shape: [batch_size x 1 x H x W]
            tiled_cluster_means += mask * cluster_means[:, :, k].unsqueeze(2).unsqueeze(
                3
            )

        # ignore label -1
        labeled_embeddings = (
            (cluster_masks >= 0).squeeze(1).float()
        )  # Shape: [batch_size x H x W]

        # Compute distance to cluster center
        if self.metric == "cosine":
            intra_cluster_distances = labeled_embeddings * (
                0.5 * (1 - torch.sum(x * tiled_cluster_means, dim=1))
            )  # Shape: [batch_size x H x W]
        elif self.metric == "euclidean":
            intra_cluster_distances = labeled_embeddings * (
                torch.norm(x - tiled_cluster_means, dim=1)
            )
        ...
        if torch.sum(intra_cluster_mask) > 0:
            ...
            datapoint_weights = torch.zeros(
                (batch_size,) + intra_cluster_distances.shape[1:], device=x.device
            )
            for k in range(K):
                # find number of datapoints in cluster k that are > alpha away from cluster center
                mask = (
                    (cluster_masks == k).float().squeeze(1)
                )  # Shape: [batch_size x H x W]
                N_k = torch.sum(
                    (intra_cluster_distances > self.alpha).float() * mask,
                    dim=[1, 2],
                    keepdim=True,
                )  # Shape: [batch_size x 1 x 1]
                datapoint_weights += mask * N_k
            datapoint_weights = torch.max(
                datapoint_weights, torch.FloatTensor([50]).to(x.device)
            )  # Max it with 50 so it doesn't get too small
            datapoint_weights *= K

            intra_cluster_loss = (
                torch.sum(intra_cluster_loss / datapoint_weights) / batch_size
            )
        else:
            intra_cluster_loss = torch.sum(
        ...
        if K > 1:
            if self.metric == "cosine":
                ...
                inter_cluster_distances = 0.5 * (
                    1
                    - torch.sum(
                        cluster_means.unsqueeze(2) * cluster_means.unsqueeze(3), dim=1
                    )
                )
            elif self.metric == "euclidean":
            ...
            inter_cluster_loss = torch.sum(
                torch.pow(
                    torch.clamp(
                        zero_diagonal(self.delta - inter_cluster_distances), min=0
                    ),
                    2,
                )
            ) / (K * (K - 1) / 2 * batch_size)
