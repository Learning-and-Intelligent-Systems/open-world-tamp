...
def compute_xyz(depth_img, camera_params):
    ...
    If focal lengths fx,fy are stored in the camera_params dictionary,
    use that. Else, assume camera_params contains parameters used to
    generate synthetic data (e.g. fov, near, far, etc)
    ...
    indices = util_.build_matrix_of_indices(
...
class TableTopObject(data.Dataset, datasets.imdb):
    def __init__(self, image_set, tabletop_object_path=None):
        ...
        self._tabletop_object_path = (
            ...
            if tabletop_object_path is None
    ...
    def _get_default_path(self):
        """Return the default path where tabletop_object is expected to be
        installed."""
