...
class ImageListener:
    def __init__(self, network, network_crop):
        ...
        self.label_refined_pub = rospy.Publisher(
            "seg_label_refined", Image, queue_size=10
        )
        self.image_pub = rospy.Publisher("seg_image", Image, queue_size=10)
        self.image_refined_pub = rospy.Publisher(
            "seg_image_refined", Image, queue_size=10
        )
        ...
        if cfg.TEST.ROS_CAMERA == "D415":
            ...
            self.base_frame = "measured/base_link"
            rgb_sub = message_filters.Subscriber(
                "/k4a/rgb/image_raw", Image, queue_size=10
            )
            depth_sub = message_filters.Subscriber(
                "/k4a/depth_to_rgb/image_raw", Image, queue_size=10
            )
            ...
            rgb_sub = message_filters.Subscriber(
                "/%s/rgb/image_color" % (cfg.TEST.ROS_CAMERA), Image, queue_size=10
            )
            depth_sub = message_filters.Subscriber(
                "/%s/depth_registered/image" % (cfg.TEST.ROS_CAMERA),
                Image,
                queue_size=10,
            )
            msg = rospy.wait_for_message(
                "/%s/rgb/camera_info" % (cfg.TEST.ROS_CAMERA), CameraInfo
            )
    ...
    def run_network(self):
        ...
        if cfg.INPUT == "DEPTH" or cfg.INPUT == "RGBD":
            ...
            xyz_img = compute_xyz(
                depth_img, self.fx, self.fy, self.px, self.py, height, width
            )
            depth_blob = torch.from_numpy(xyz_img).permute(2, 0, 1)
            sample["depth"] = depth_blob.unsqueeze(0)

        out_label, out_label_refined = test_sample(
            sample, self.network, self.network_crop
        )
        ...
        if out_label_refined is not None:
            ...
            label_msg_refined = self.cv_bridge.cv2_to_imgmsg(
                label_refined.astype(np.uint8)
            )
        ...
        im_label = visualize_segmentation(
            im_color[:, :, (2, 1, 0)], label, return_rgb=True
        )
        ...
        if out_label_refined is not None:
            im_label_refined = visualize_segmentation(
                im_color[:, :, (2, 1, 0)], label_refined, return_rgb=True
            )
...
def parse_args():
    ...
    parser.add_argument(
        "--instance",
        dest="instance_id",
        help="PoseCNN instance id to use",
        default=0,
        type=int,
    )
    parser.add_argument(
