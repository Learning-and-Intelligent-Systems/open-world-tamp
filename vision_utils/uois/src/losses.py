...
class WeightedLoss(nn.Module):
    ...
    def generate_weight_mask(self, mask, to_ignore=None):
        """Generates a weight mask where pixel weights are inversely
        proportional to how many pixels are in the class.
...
class CELossWeighted(WeightedLoss):
    ...
    def forward(self, x, target):
        ...
        @param x: a [N x C x H x W] torch.FloatTensor of values @param
        target: a [N x H x W] torch.LongTensor of values
        """
...
class BCEWithLogitsLossWeighted(WeightedLoss):
    ...
    def forward(self, x, target):
        ...
        @param target: a [N x H x W] torch.FloatTensor of values in [0,
        1]
        """
