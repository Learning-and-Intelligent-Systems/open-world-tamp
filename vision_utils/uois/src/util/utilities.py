...
def imwrite_indexed(filename, array):
    ...
    palette_abspath = (
        "/home/chrisxie/projects/random_stuff/palette.txt"  # hard-coded filepath
    )
...
def mask_to_tight_box(mask):
    if type(mask) == torch.Tensor:
        ...
        raise Exception(
            f"Data type {type(mask)} not understood for mask_to_tight_box..."
        )


def compute_xyz(depth_img, camera_params):
    """Compute ordered point cloud from depth image and camera parameters.
    Assumes camera uses left-handed coordinate system, with x-axis pointing
    right y-axis pointing up z-axis pointing "forward".

    @param depth_img: a [H x W] numpy array of depth values in meters
    @param camera_params: a dictionary with parameters of the camera
    used
    ...
    indices = build_matrix_of_indices(
        camera_params["img_height"], camera_params["img_width"]
    )
    indices[..., 0] = np.flipud(
        indices[..., 0]
...
def seg2bmap(seg, return_contour=False):
    ...
    boundaries. This boundary lives on the mask, i.e. it's a subset of the
    mask.

    @param seg: a [H x W] numpy array of values in {0,1}

    @return: a [H x W] numpy array of values in {0,1}          a [2 x
    num_boundary_pixels] numpy array. [0,:] is y-indices, [1,:] is
    x-indices
    """
...
def torch_to_numpy(torch_tensor, is_standardized_image=False):
    ...
    If it's an rgb image, it puts it back in [0,255] range (and undoes
