...
def unstandardize_image(image):
    """Convert a numpy.ndarray [H x W x 3] standardized image back to RGB (type
    np.uint8) Inverse of standardize_image()
    ...
    while not valid_transform:
        ...
        while morphology_kernel_size == 0:
            if num_ksize_tries >= 50:  # 50 tries for this
                print(
                    f"Morph: Exhausted number of augmentation tries... Sidelength: {sidelength}"
                )
                return label

            dilation_percentage = np.random.beta(
                noise_params["label_dilation_alpha"],
...
def random_ellipses(label, noise_params):
    ...
    while not valid_transform:
        ...
        scale_factor = (
            max(x_max - x_min, y_max - y_min) * noise_params["ellipse_size_percentage"]
        )  # Mean of gamma r.v.
        x_radii = np.random.gamma(
            noise_params["ellipse_gamma_base_shape"] * scale_factor,
...
def random_translation(label, noise_params):
    ...
    while not valid_transform:
        ...
        translation_percentage = np.random.beta(
            noise_params["translation_alpha"], noise_params["translation_beta"]
        )
        translation_percentage = max(
            translation_percentage, noise_params["translation_percentage_min"]
        )
...
def random_rotation(label, noise_params):
    ...
    while not valid_transform:
        ...
        applied_angle = np.random.uniform(
            -noise_params["rotation_angle_max"], noise_params["rotation_angle_max"]
        )
...
def random_cut(label, noise_params):
    ...
    while not valid_transform:
        ...
        cut_percentage = np.random.uniform(
            noise_params["cut_percentage_min"], noise_params["cut_percentage_max"]
        )
...
def random_add(label, noise_params):
    ...
    while not valid_transform:
        ...
        add_percentage = np.random.uniform(
            noise_params["add_percentage_min"], noise_params["add_percentage_max"]
        )
