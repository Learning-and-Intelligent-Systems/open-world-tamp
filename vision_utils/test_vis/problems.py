...
from examples.pybullet.utils.pybullet_tools.pr2_problems import (
    create_box, create_floor, create_kitchen, create_pr2, set_point, unit_pose)
from examples.pybullet.utils.pybullet_tools.pr2_utils import (
    CENTER_LEFT_ARM, REST_LEFT_ARM, SIDE_HOLDING_LEFT_ARM,
    TOP_HOLDING_LEFT_ARM, WIDE_LEFT_ARM, arm_conf, close_arm, create_gripper,
    get_carry_conf, get_other_arm, open_arm, set_arm_conf)
...
class Voxelgrid(object):
    ...
    def create_virtual_box(self, pose=unit_pose(), color=np.zeros(4)):
        # color = (1,0,1,0.3)
        box = create_virtual_box(
            self.resolution, self.resolution, self.resolution, color=color
        )
        set_point(box, pose)
        return box
    ...
    def check_collision(
        ...
        collide_visray = False
        block_list = {}
        can_clear = []
        invalid_list = [-1, 0, 1, 2]  # TODO adhoc. invalid objects in get_image.segment
        object_list = {e.obj_id: e for e in entity_list if e.obj_id not in invalid_list}
        for voxel in voxels:
            if voxel not in self.occupied:  # has been observed
                continue
            # if voxel not in self.box.keys():
            #     self.box[voxel] = self.create_virtual_box(self.center_from_voxel(voxel))
            collision_voxel = self.create_box(self.center_from_voxel(voxel))
            if body_collision(collision_voxel, body):
                # print(f'Collision! Voxel {voxel} with collision_voxel {collision_voxel}, virtual box id {self.box[voxel]}')
                if (
                    occlusion
                ):  # and #not ray_collision((camera_pos,self.center_from_voxel(voxel))):#not single_collision(add_line(camera_pos,self.center_from_voxel(voxel))):
                    # TODO collision check of (eye,target)
                    tgt = self.center_from_voxel(voxel)
                    diff = np.asarray(list(tgt)) - np.asarray(list(camera_pos))
                    diff_len = (diff**2).sum() ** 0.5
                    tilt = np.arcsin(-diff[2] / diff_len)
                    pan = np.arctan(diff[1] / diff[0])
                    vis_ray = create_cylinder(0.01, diff_len)
                    center = np.divide(np.add(tgt, camera_pos), 2.0)
                    set_point(vis_ray, center)
                    set_euler(vis_ray, (0, np.pi / 2.0 + tilt, pan))
                    # line = add_line(camera_pos,self.center_from_voxel(voxel))
                    for body2 in object_list.keys():
                        if body_collision(body2, vis_ray):
                            # print(f'Collide. BODY2 is {body2}')
                            collide_visray = True
                            if object_list[body2] not in block_list.keys():
                                block_list[object_list[body2]] = [
                                    [object_list[body2].loc, self.poses[voxel]]
                                ]
                            else:
                                block_list[object_list[body2]].append(
                                    [object_list[body2].loc, self.poses[voxel]]
                                )
                            break

                    if not collide_visray:
                        can_clear.append(voxel)
                    collide_visray = False
                    remove_body(vis_ray)
                else:
...
def get_localized_rooms(task, **kwargs):
    ...
    for body in task.surfaces + task.movable:
        set_uniform_belief(task, b_on, body)  # , **kwargs) #adhoc TODO
...
class VisTask(object):
    def __init__(
        ...
        print(f"goal category is {goal_category}")

    def get_bodies(self):
        return list(self.movable + self.surfaces + self.rooms)  #
...
def get_kitchen_task(arm="left", grasp_types="top", goal_category=None, **kwargs):
    with HideOutput():
        pr2 = create_pr2(use_drake=USE_DRAKE_PR2)
    set_arm_conf(
        pr2, arm, arm_conf(arm, WIDE_LEFT_ARM)
    )  # get_carry_conf(arm, grasp_type))
    open_arm(pr2, arm)  # set_gripper
    other_arm = get_other_arm(arm)
    set_arm_conf(
        pr2, other_arm, arm_conf(other_arm, WIDE_LEFT_ARM)
    )  # TOP_HOLDING_LEFT_ARM | SIDE_HOLDING_LEFT_ARM | REST_LEFT_ARM | WIDE_LEFT_ARM | CENTER_LEFT_ARM
    open_arm(pr2, other_arm)  # set gripper

    # table, cabbage_list, sink, stove = create_random_kitchen()
    floor = create_floor()
    # table = load_pybullet(f"{URDF_PATH}/dinnerware/plate.urdf")

    w, h = 0.5, 0.55
    table = create_box(w, w * 2, h, color=(0.75, 0.75, 0.75, 1))
    set_point(table, (0.4 + w / 2.0, 0, h / 2))
    ...
    grid = Voxelgrid(0.05, (0.4, -w, h), (w + 0.4, w, h + 0.3))
    ...
    return VisTask(
        robot=pr2,
        arms=[arm],
        grasp_types=[grasp_types] if type(grasp_types) == str else grasp_types,
        surfaces=surfaces,
        rooms=rooms,
        vspace=grid,
...
def get_problem1(localized="rooms", goal_category=5, **kwargs):
    task = get_kitchen_task(goal_category=goal_category, **kwargs)
    if localized == "rooms":
        belief_state = get_localized_rooms(task, **kwargs)
    ...
    task.state = belief_state
    return task, belief_state
