...
def shoot_rays(args):
    ...
    for obj_class in classes:
        ...
        obj_instances = sorted(
            [
                x
                for x in next(os.walk(join(args.shapenet_path, obj_class)))[1]
                if "ply" not in x
            ]
        )
        ...
        instance_paths = [
            join(args.shapenet_path, obj_class, obj_instance, "model.obj")
        ...
        _ = Parallel(n_jobs=-1, backend="multiprocessing")(
            delayed(sample_instance)(*i)
            for i in zip(instance_paths, [args.virtualscan] * len(instance_paths))
        )
...
def main():
    ...
    parser.add_argument(
        "--virtualscan",
        type=str,
        help="Path to the virtual scanner executable",
