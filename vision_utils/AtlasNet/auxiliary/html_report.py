...
def main(trainer, outHtml=None):
    ...
    webpage = HtmlGenerator.HtmlGenerator(
        path=outHtml, title=trainer.opt.dir_name, local_copy=True
    )
    ...
    webpage.add_title(
        f"Reconstruction (Val):{loss_val} Metro:{trainer.metro_results} Fscore:{final_fscore}"
    )
    ...
    curve_recons = webpage.chart(
        trainer.html_report_data.data_curve,
        title="Reconstruction quality - chamfer log",
    )
    table2.add_row([curve_recons])
    curve_recons = webpage.chart(
        trainer.html_report_data.fscore_curve, title="Reconstruction quality - fscore"
    )
    table2.add_row([curve_recons])
    ...
    for i in range(3):
        ...
        table.add_row(
            [
                webpage.image(output_mesh["image_path"]),
    ...
    webpage_after = HtmlGenerator.HtmlGenerator(
        path="master.html", reload_path="master.pkl"
    )
