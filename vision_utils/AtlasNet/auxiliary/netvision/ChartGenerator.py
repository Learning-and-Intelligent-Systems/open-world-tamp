...
class Chart:
    ...
    def __str__(self):
        if self.chart_type == "scatter":
            ...
            mini, maxi = min([min(v) for k, v in self.data.items()]), max(
                [max(v) for k, v in self.data.items()]
            )
            ...
            "datasets": [
                {
                    ...
                    "xLabels": [
                        x + 1
                        for x in range(len(self.data[[x for x in self.data.keys()][0]]))
                    ],
                    "borderColor": self.colors[i % len(self.colors)],
                    "backgroundColor": self.colors[
                        (i + (self.chart_type != "line") * 1) % len(self.colors)
                    ],
                    "borderWidth": 1,
                }
        ...
        if self.x_labels is None:
            data_real_dict["labels"] = [
                x + 1 for x in range(len(self.data[[x for x in self.data.keys()][0]]))
            ]
        else:
        ...
        options_dict = {
            "scales": scales_dict,
            "legend": {"labels": {"fontColor": self.font_color}},
        }
...
class ChartGenerator:
    ...
    def make_chart(
        ...
        self.chart_it += 1
        return Chart(
            data, font_color, chart_type, title, width_factor, x_labels, self.chart_it
        )
