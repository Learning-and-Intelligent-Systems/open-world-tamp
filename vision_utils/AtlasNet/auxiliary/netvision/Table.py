...
class Table:
    def __init__(self, title):
        ...
        self.row_title_weight = 5.0

    def add_column(self, title):
        if self.are_columns_fixed:
            raise TableException(
                "You can't add a column after having added the first row!"
            )
    ...
    def add_row(self, data, title=""):
        ...
        padding = (len(self.columns) - len(data) % len(self.columns)) % len(
            self.columns
        )
        ...
        for i in range(nb_rows_to_add):
            ...
            if i == nb_rows_to_add - 1:
                row_data += [""] * padding
            row_str += f"</td>\n{self.td_str}".join(
                [str(self._pretreat_data(x)) for x in row_data]
            )
            row_str += "</td>\n</tr>\n"
    ...
    def _pretreat_data(self, data):
        if type(data) is Chart:
            data.width = (
                f"({data.width_factor*self.width_percentage*self.row_title_weight}"
                f'*window.innerWidth*0.01).toString() + "px"'
            )
        return data

    def _make_td_str(self):
        self.width_percentage = 100.0 / float(
            len(self.columns) * self.row_title_weight + 1
        )
