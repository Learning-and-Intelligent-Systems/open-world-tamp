...
class ConfusionMatrixGenerator:
    ...
    def make_confusionmatrix(
        self, data, rows_titles, colums_titles, title="Confusion", colormap=None
    ):
        ...
        outstr += (
            f'<td  style="text-align: center; font-weight: bold;" >Colormap</td>\n'
        )
        outstr += "</tr\n>"
        ...
        for i in range(data.shape[0]):
            ...
            index_rescaled = int(
                (float(i) / float(data.shape[0])) * (np.max(data) - np.min(data))
                + np.min(data)
            )
