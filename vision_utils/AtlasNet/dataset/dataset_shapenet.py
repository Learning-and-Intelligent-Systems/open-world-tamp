...
class ShapeNet(data.Dataset):
    """Shapenet Dataloader Uses Shapenet V1 Make sure to respect shapenet
    Licence.
                ...
                if len(list_pointcloud) != 0:
                    ...
                    for pointcloud in list_pointcloud:
                        ...
                        if not self.opt.SVR or exists(image_path):
                            self.category_datapath[category].append(
    ...
    def init_normalization(self):
        ...
        if self.opt.normalization == "UnitBall":
            self.normalization_function = (
                pointcloud_processor.Normalization.normalize_unitL2ball_functional
            )
        elif self.opt.normalization == "BoundingBox":
            self.normalization_function = (
                pointcloud_processor.Normalization.normalize_bounding_box_functional
            )
        else:
            self.normalization_function = (
                pointcloud_processor.Normalization.identity_functional
            )
    ...
    def __getitem__(self, index):
        ...
        if self.opt.SVR:
            if self.train:
                ...
                im = Image.open(
                    join(return_dict["image_path"], ShapeNet.int2str(N) + ".png")
                )
                im = self.dataAugmentation(im)  # random crop
            else:
                im = Image.open(
                    join(
                        return_dict["image_path"],
                        ShapeNet.int2str(self.idx_image_val) + ".png",
                    )
                )
