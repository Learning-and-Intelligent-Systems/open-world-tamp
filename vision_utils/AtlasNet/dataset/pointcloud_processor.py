...
class Operation(object):
    def __init__(self, points, inplace=True, keep_track=False):
        """The keep track boolean is used in case one wants to unroll all the
        operation that have been performed :param keep_track: boolean."""
    ...
    def get_3D_rot_matrix(axis, rad_angle):
        """Get a 3D rotation matrix around axis with angle in radian :param
        axis: int :param angle: torch.tensor of size Batch.
...
class Normalization(Operation):
    ...
    def center_pointcloud(self):
        """In-place centering :param points:  Tensor Batch, N_pts, D_dim
        :return: None."""
        # input :
    ...
    def normalize_unitL2ball(self):
        """In-place normalization of input to unit ball :param points: torch
        Tensor Batch, N_pts, D_dim :return: None."""
...
class DataAugmentation(Operation):
    ...
    def random_axial_rotation(self, axis=0, normals=False, range_rot=360):
        """Compute a random rotation of the batch around an axis.

        There is is no in-place version of this function because bmm_ is
        not possible in pytorch.
        :param points: torch Tensor Batch, N_pts, D_dim
    ...
    def random_rotation(self, normals=False):
        """Compute a random rotation of the batch.
    ...
    def random_flips(self, dims=[]):
        """In place Random flip :param points: torch Tensor Batch, N_pts, D_dim
        :return:"""
