...
class TrainerIteration(object):
    """This class implements all functions related to a single forward pass of
    Atlasnet.
    ...
    def make_network_input(self):
        ...
        if self.opt.SVR:
            ...
            self.data.network_input = (
                self.data.points.transpose(2, 1).contiguous().to(self.opt.device)
            )

    def common_ops(self):
        """Commom operations between train and eval forward passes :return:"""
        self.make_network_input()
        self.batch_size = self.data.points.size(0)

        self.data.pointsReconstructed_prims = self.network(
            self.data.network_input, train=self.flags.train
        )
    ...
    def visualize(self):
        if self.iteration % 50 == 1:
            ...
            self.visualizer.show_pointcloud(
                self.data.points[0], title=f"GT {tmp_string}"
            )
            self.visualizer.show_pointcloud(
                self.data.pointsReconstructed[0], title=f"Reconstruction {tmp_string}"
            )
            if self.opt.SVR:
                self.visualizer.show_image(
                    self.data.image[0], title=f"Input Image {tmp_string}"
                )

    def test_iteration(self):
        """Forward evaluation pass :return:"""
        self.common_ops()
        ...
        print(
            ...
            + colored(
                ...
                % (
                    self.epoch,
                    self.iteration,
                    self.datasets.len_dataset_test / self.opt.batch_size_test,
                ),
                "red",
            )
