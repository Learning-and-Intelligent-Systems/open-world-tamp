...
def isolate_files():
    """Utility fonction to generate the metro_file archive.

    Useless to all users but the author.
    """
    ...
    for file in files:
        if file[-3:] == "ply":
            ...
            pymesh.save_mesh(
                "/".join([".", "dataset", "data", "metro_files", cat, name + ".ply"]),
                mesh,
                ascii=True,
            )
            np.save(
                "/".join([".", "dataset", "data", "metro_files", cat, name + ".npy"]),
                points,
            )
            copy(
                path_png,
                "/".join([".", "dataset", "data", "metro_files", cat, name + ".png"]),
            )
...
def main():
    ...
    parser.add_argument(
        "--metro",
        type=str,
        help="Path to the metro executable",
