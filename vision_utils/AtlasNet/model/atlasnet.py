...
from model.model_blocks import Identity, Mapping2Dto3D
...
class Atlasnet(nn.Module):
    ...
    def forward(self, latent_vector, train=True):
        ...
        :param latent_vector: an opt.bottleneck size vector encoding a 3D shape
        or an image.
        ...
        if train:
            ...
            input_points = [
                self.template[i].get_regular_points(
                    self.nb_pts_in_primitive_eval, device=latent_vector.device
                )
                for i in range(self.opt.nb_primitives)
            ]
        ...
        output_points = torch.cat(
            [
                self.decoder[i](input_points[i], latent_vector.unsqueeze(2)).unsqueeze(
                    1
                )
    ...
    def generate_mesh(self, latent_vector):
        ...
        input_points = [
            self.template[i].get_regular_points(
                self.nb_pts_in_primitive, latent_vector.device
            )
            for i in range(self.opt.nb_primitives)
        ]
        ...
        # Deform each patch
        output_points = [
            self.decoder[i](input_points[i], latent_vector.unsqueeze(2)).squeeze()
            for i in range(0, self.opt.nb_primitives)
        ]

        output_meshes = [
            [
                output_points[i].transpose(1, 0).contiguous().cpu().numpy(),
                self.template[i].mesh.faces,
            ]
            for i in range(self.opt.nb_primitives)
        ]
