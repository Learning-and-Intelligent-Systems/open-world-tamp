...
GRIPPER_PC = np.load(
    "grasp/graspnet/gripper_models/panda_pc.npy", allow_pickle=True
).item()["points"]
GRIPPER_PC[:, 3] = 1.0
...
def distance_by_translation_point(p1, p2):
    """Gets two nx3 points and computes the distance between point p1 and
    p2."""
...
def perturb_grasp(
    grasp, num, min_translation, max_translation, min_rotation, max_rotation
):
    """Self explanatory."""
    output_grasps = []
    for _ in range(num):
...
def uniform_quaternions():
    ...
    quaternions = [
        [float(t[0]), float(t[1]), float(t[2]), float(t[3])] for t in quaternions
    ]
...
def merge_pc_and_gripper_pc(
    pc, gripper_pc, instance_mode=0, pc_latent=None, gripper_pc_latent=None
):
    """Merges the object point cloud and gripper point cloud and adds a binary
    auxilary feature that indicates whether each point belongs to the object or
    to the gripper."""
...
def transform_control_points(gt_grasps, batch_size, mode="qt", device="cpu"):
    ...
    element is catenation of quaternion and translation for each grasps.
    mode = 'rt': expects to have shape (batch_size x 4 x 4) where   each
    element is 4x4 transformation matrix of each grasp.
    """
...
def transform_control_points_numpy(gt_grasps, batch_size, mode="qt"):
    ...
    if mode == "qt":
        ...
        num_control_points = control_points.shape[1]
        input_gt_grasps = gt_grasps
        gt_grasps = np.expand_dims(input_gt_grasps, 1).repeat(
            num_control_points, axis=1
        )
        gt_q = gt_grasps[:, :, :4]
        gt_t = gt_grasps[:, :, 4:]
...
def quaternion_mult(q, r):
    ...
    Expects two equally-sized tensors of shape (*, 4), where * denotes
    any number of dimensions. Returns q*r as a tensor of shape (*, 4).
    """
    assert q.shape[-1] == 4
    assert r.shape[-1] == 4
...
def rotate_point_by_quaternion(point, q, device="cpu"):
    ...
    r = torch.cat(
        [torch.zeros((shape[0], shape[1], 1), dtype=point.dtype).to(device), point],
        dim=-1,
    )
...
def control_points_from_rot_and_trans(grasp_eulers, grasp_translations, device="cpu"):
...
def qeuler(q, order, epsilon=0):
    ...
    Expects a tensor of shape (*, 4), where * denotes any number of
    dimensions. Returns a tensor of shape (*, 3).
    """
    assert q.shape[-1] == 4
...
def choose_grasps_better_than_threshold_in_sequence(
    eulers, translations, probs, threshold=0.7
):
    """Chooses the grasps with the maximum score in the sequence of grasp
    refinements."""
...
def qrot(q, v):
    ...
    Expects a tensor of shape (*, 4) for q and a tensor of shape (*, 3)
    for v, where * denotes any number of dimensions. Returns a tensor of
    shape (*, 3).
    """
    assert q.shape[-1] == 4
    assert v.shape[-1] == 3
...
def get_inlier_grasp_indices(grasp_list, query_point, threshold=1.0, device="cpu"):
    """This function returns all grasps whose distance between the mid of the
    finger tips and the query point is less than the threshold value.
    ...
    for grasps in grasp_list:
        grasp_cps = transform_control_points(grasps, grasps.shape[0], device=device)
