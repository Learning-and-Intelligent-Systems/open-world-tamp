...
def define_classifier(opt, gpu_ids, arch, init_type, init_gain, device):
    ...
    if arch == "vae":
        ...
        net = GraspEvaluator(
            opt.model_scale, opt.pointnet_radius, opt.pointnet_nclusters, device
        )
    else:
...
class GraspSampler(nn.Module):
    ...
    def create_decoder(
        self, model_scale, pointnet_radius, pointnet_nclusters, num_input_features
    ):
        # The number of input features for the decoder is 3+latent space where 3
        # represents the x, y, z position of the point-cloud

        self.decoder = base_network(
            pointnet_radius, pointnet_nclusters, model_scale, num_input_features
        )
...
class GraspSamplerVAE(GraspSampler):
    ...
    def __init__(
        ...
        self.create_decoder(
            model_scale, pointnet_radius, pointnet_nclusters, latent_size + 3
        )
    ...
    def create_encoder(
        ...
        self.encoder = base_network(
            pointnet_radius, pointnet_nclusters, model_scale, 19
        )
    ...
    def forward_train(self, pc, grasp):
        input_features = (
            torch.cat((pc, grasp.unsqueeze(1).expand(-1, pc.shape[1], -1)), -1)
            .transpose(-1, 1)
            .contiguous()
        )
    ...
    def generate_dense_latents(self, resolution):
        """For the VAE sampler we consider dense latents to correspond to those
        between -2 and 2."""
        latents = torch.meshgrid(
            *[torch.linspace(-2, 2, resolution) for i in range(self.latent_size)]
        )
        return torch.stack(
            [latents[i].flatten() for i in range(len(latents))], dim=-1
        ).to(self.device)


class GraspSamplerGAN(GraspSampler):
    """Altough the name says this sampler is based on the GAN formulation, it
    is not actually optimizing based on the commonly known adversarial game.

    Instead, it is based on the Implicit Maximum Likelihood Estimation
    from
    https://arxiv.org/pdf/1809.09087.pdf
    which is similar to the GAN formulation
...
class GraspEvaluator(nn.Module):
    def __init__(
        self, model_scale=1, pointnet_radius=0.02, pointnet_nclusters=128, device="cpu"
    ):
    ...
    def create_evaluator(self, pointnet_radius, model_scale, pointnet_nclusters):
        ...
        self.evaluator = base_network(
            pointnet_radius, pointnet_nclusters, model_scale, 4
        )
    ...
    def merge_pc_and_gripper_pc(self, pc, gripper_pc):
        """Merges the object point cloud and gripper point cloud and adds a
        binary auxiliary feature that indicates whether each point belongs to
        the object or to the gripper."""
        pc_shape = pc.shape
