...
def control_point_l1_loss_better_than_threshold(
    pred_control_points,
    gt_control_points,
    confidence,
...
def accuracy_better_than_threshold(
    pred_success_logits, gt, confidence, confidence_threshold, device="cpu"
):
    """Computes average precision for the grasps with confidence >
    threshold."""
    ...
    negative_acc = torch.sum(correct * mask * (1.0 - gt)) / torch.max(
        torch.sum(mask * (1.0 - gt)), torch.tensor(1)
    )
...
def classification_with_confidence_loss(
    pred_logit, gt, confidence, confidence_weight, device="cpu"
):
    """Computes the cross entropy loss and confidence term that penalizes
    outputing zero confidence.

    Returns cross entropy loss and the confidence regularization term.
    """
...
def min_distance_loss(
    ...
    error = torch.sum(torch.abs(error), -1)  # L1 distance of error (N_pred, N_gt, M)
    error = torch.mean(
        error, -1
    )  # average L1 for all the control points. (N_pred, N_gt)
    ...
    if confidence is not None:
        ...
        selected_confidence = torch.nn.functional.one_hot(
            closest_index, num_classes=closest_index.shape[0]
        ).float()  # (N_gt, N_pred)
        ...
        confidence_term = (
            torch.mean(torch.log(torch.max(confidence, torch.tensor(1e-4).to(device))))
            * confidence_weight
        )
    else:
        confidence_term = 0.0
...
def min_distance_better_than_threshold(
    pred_control_points,
    gt_control_points,
    confidence,
    confidence_threshold,
...
def kl_divergence(mu, log_sigma, device="cpu"):
    ...
    return torch.mean(
        -0.5 * torch.sum(1.0 + log_sigma - mu**2 - torch.exp(log_sigma), dim=-1)
    )
