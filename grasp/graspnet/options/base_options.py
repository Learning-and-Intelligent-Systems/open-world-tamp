...
class BaseOptions:
    ...
    def initialize(self):
        ...
        self.parser.add_argument(
            "--num_objects_per_batch", type=int, default=1, help="data batch size."
        )
        self.parser.add_argument("--num_grasps_per_object", type=int, default=64)
        self.parser.add_argument(
            "--npoints", type=int, default=1024, help="number of points in each batch"
        )
        self.parser.add_argument(
            "--occlusion_nclusters",
            type=int,
        ...
        self.parser.add_argument(
            "--depth_noise", type=float, default=0.0
        )  # to be used in the data reader.
        self.parser.add_argument("--num_grasp_clusters", type=int, default=32)
        self.parser.add_argument(
            "--arch", choices={"vae", "gan", "evaluator"}, default="vae"
        )
        self.parser.add_argument(
            "--max_dataset_size",
            type=int,
    ...
    def parse(self):
        ...
        self.opt.batch_size = (
            self.opt.num_objects_per_batch * self.opt.num_grasps_per_object
        )
        ...
        if self.is_train:
            ...
            name += (
                "_lr_"
                + str(self.opt.lr).split(".")[-1]
                + "_bs_"
                + str(self.opt.batch_size)
            )
            name += (
                "_scale_"
                + str(self.opt.model_scale)
                + "_npoints_"
                + str(self.opt.pointnet_nclusters)
                + "_radius_"
                + str(self.opt.pointnet_radius).split(".")[-1]
            )
