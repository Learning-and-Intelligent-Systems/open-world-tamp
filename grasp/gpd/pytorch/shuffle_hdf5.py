...
with h5py.File(sys.argv[2], "w") as f:
    images_dset = f.create_dataset(
        "images", images_shape, dtype="uint8", compression="gzip", compression_opts=4
    )
    labels_dset = f.create_dataset(
        "labels", labels_shape, dtype="uint8", compression="gzip", compression_opts=4
    )

    for i in range(len(subIdx) - 1):
        print("Block %d/%d ..." % (i + 1, len(subIdx) - 1))
        images = np.empty(
            (0, images_shape[1], images_shape[2], images_shape[3]), np.uint8
        )
        ...
        for j in range(len(subIdx) - 1):
            print(
                "  Loading images and labels from block %d/%d ..."
                % (j + 1, len(subIdx) - 1)
            )
            with h5py.File(sys.argv[1], "r") as h5_file:
            ...
            indices = idx2[
                np.where(np.logical_and(idx2 >= subIdx[j], idx2 < subIdx[j + 1]))[0]
            ]
