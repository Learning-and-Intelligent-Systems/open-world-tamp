...
with h5py.File(sys.argv[2]) as h5_file_out:
    ...
    dset_small = h5_file_out.create_dataset(
        "images", data=dset.data[:num], compression="gzip", compression_opts=4
    )
    print("Storing labels ...")
    dset_small = h5_file_out.create_dataset(
        "labels", data=dset.target[:num], compression="gzip", compression_opts=4
    )
    print(len(dset))
