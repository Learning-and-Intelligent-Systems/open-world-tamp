...
with h5py.File(sys.argv[2]) as h5_file_out:
    ...
    dset_images = h5_file_out.create_dataset(
        "images",
        [num] + list(images.shape[1:]) + [images[0, 0, 0].shape[0]],
        compression="gzip",
        compression_opts=4,
    )
    dset_labels = h5_file_out.create_dataset(
        "labels", [num] + list(labels.shape[1:]), compression="gzip", compression_opts=4
    )

    for i in range(iters):
        print("i: %d" % i)
        print("Copying %d images ..." % (indices[i + 1] - indices[i]))
        dset_images[indices[i] : indices[i + 1], :, :] = images[
            indices[i] : indices[i + 1], :, :
        ]
        print("Copying %d labels ..." % (indices[i + 1] - indices[i]))
        dset_labels[indices[i] : indices[i + 1], :] = labels[
            indices[i] : indices[i + 1], :
        ]
