...
with h5c.File(fin, "r", chunk_cache_mem_size=CHUNK_CACHE) as f:
    ...
    with h5c.File("temp.h5", "w", chunk_cache_mem_size=CHUNK_CACHE) as db:
        for i in range(m):
            db.create_dataset(
                "images" + str(i),
                (max_in_memory,) + images_shape[1:],
                dtype="uint8",
                chunks=chunk_shape,
            )
            db.create_dataset(
                "labels" + str(i), (max_in_memory,) + labels_shape[1:], dtype="uint8"
            )
    ...
    with h5c.File(fout, "w", chunk_cache_mem_size=CHUNK_CACHE) as fout:
        images_out = fout.create_dataset(
            "images", images_shape, dtype="uint8", chunks=chunk_shape, compression="lzf"
        )
        ...
        with h5c.File("temp.h5", "r", chunk_cache_mem_size=CHUNK_CACHE) as fin:
            for i in range(m):
                ...
                k = len(p)
                print(
                    "%d/%d: %d to %d. k: %d" % (i + 1, m, blocks[i], blocks[i + 1], k)
                )
