# Author: Caelan-->Aidan
# Modified from pybullet_planning to work for robot models without tool frame
# Another solution is to add the tool frame to panda and all other robots without it.
# However, this breaks IK fast for some unknown rason.
...
import numpy as np
from pybullet_tools.ikfast.utils import (
...
# def get_base_from_ee(robot, ikfast_info, tool_link, world_from_target, arm):
#     world_from_base = get_link_pose(robot, link_from_name(robot, ikfast_info.base_link))
#     return multiply(invert(world_from_base), world_from_target, robot.get_parent_from_tool(arm))
...
def ikfast_inverse_kinematics(
    ...
    **kwargs
    ...
    base_from_ee = get_base_from_ee(
        robot, ikfast_info, tool_link, world_from_target, arm
    )
...
def closest_inverse_kinematics(
    ...
    **kwargs
    ...
    generator = ikfast_inverse_kinematics(
        robot, ikfast_info, tool_link, world_from_target, arm=arm, norm=norm, **kwargs
    )
