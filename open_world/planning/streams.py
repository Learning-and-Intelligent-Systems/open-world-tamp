...
from open_world.planning.samplers import (
    COLLISION_DISTANCE,
    MOVABLE_DISTANCE,
    SELF_COLLISIONS,
...
def get_grasp_candidates(robot, obj, grasp_mode="mesh", gripper_width=INF, **kwargs):
    ...
    # TODO: add to fixed if cannot grasp

    # returns tool_from_obj
    if grasp_mode == "top":
        return randomize(
            get_top_grasps(
                obj,
    ...
    if grasp_mode == "mesh":
        # TODO: use the antipodal distance to determine the gripper width
...
def get_cfree_traj_pose_test(robot, **kwargs):
    def test(arm, sequence, obj2, pose2):
        ...
        # if obj2 in pose1.ancestors():
        #     return True
        if sequence.name.startswith('pick') and (pose2.important and not obj2.is_fragile):
            return True
...
def get_plan_mobile_pick_fn(robot, max_attempts=5, **kwargs):
    ...
    def fn(arm, obj, pose, grasp):
        ...
        while(True):
            ...
            movable_joint_names = get_joint_names(robot, movable_joints, client=robot.client)
            pose.assign()
            ...
            while(positions is None):
                ...
                if(num_attempts>max_attempts):
                    return None
            position_dict = {joint_name: joint_value for joint_name, joint_value in zip(movable_joint_names, positions)}
            arm_group_joints = robot.get_group_joints(arm)
            arm_joint_names = get_joint_names(robot, arm_group_joints, client=robot.client)


            base_group_joints = robot.get_group_joints(robot.base_group)
            base_joint_names = get_joint_names(robot, base_group_joints, client=robot.client)

            arm_positions = [position_dict[joint_name] for joint_name in arm_joint_names]
            base_positions = [position_dict[joint_name] for joint_name in base_joint_names]

            arm_conf = GroupConf(robot, arm, positions=arm_positions, client=robot.client)
            base_conf = GroupConf(robot, robot.base_group, positions=base_positions, client=robot.client)
...
def get_plan_inspect_fn(robot, environment=[], **kwargs):
    robot_saver = BodySaver(robot, client=robot.client)
    # relative_base = Pose(Point(x=0.4, z=1.), euler=Euler(pitch=PI/2)) # downward
    # relative_base = Pose(Point(x=0.4, z=1.2), euler=Euler(pitch=-PI/2)) # upward
    # relative_base = Pose(Point(x=0.4, z=1.), euler=Euler(pitch=-PI)) # sideways
    relative_camera = Pose(Point(y=0.1, z=0.5), euler=Euler(yaw=PI / 2))
    # TODO: plan for the object orientation instead

    def fn(arm, obj, grasp, base_conf):
        # TODO: don't necessarily need the grasp
        # TODO: rotate the last joint to fully scan and take into account visibility
        robot_saver.restore()
        base_conf.assign()
        obstacles = list(environment)

        side = robot.side_from_arm(robot)
        _, gripper_group, _ = robot.manipulators[side]
        gripper = robot.get_component(gripper_group)
        parent_from_tool = robot.get_parent_from_tool(side)
        [camera] = robot.cameras

        # TODO: get_base_pose in pr2_utils
        # gripper_pose = multiply(get_link_pose(robot, link_from_name(robot, 'base_footprint')), relative_base)
        gripper_pose = multiply(camera.get_pose(), relative_camera)
        ...
        last_joint = arm_conf.joints[-1]
        assert is_circular(robot, last_joint)

        start_conf = np.array([arm_conf.values[-1]])
        end_conf = start_conf + 2 * PI
        inspect_waypoints = [
            wrap_angle(convex_combination(start_conf, end_conf, w))
            for w in np.linspace(start=0.0, stop=1.0, num=4, endpoint=False)
        ]
        inspect_trajs = [
            Trajectory(robot, [last_joint], last_pair, client=robot.client)
            for last_pair in get_wrapped_pairs(inspect_waypoints)
        ]
        inspect_oobbs = []
        for inspect_traj in inspect_trajs:
            inspect_traj.first().assign()
            attachment.assign()
            inspect_oobb = get_view_oobb(obj, camera.get_pose())
            inspect_oobbs.append(
                OOBB(buffer_aabb(inspect_oobb.aabb, buffer=1e-2), inspect_oobb.pose)
            )

        sequence = Sequence(
            commands=[
                Inspect(
                    camera, tool_link, inspect_oobbs, inspect_trajs, known=environment
                )
            ],
            name="inspect-{}-{}".format(robot.side_from_arm(arm), obj),
        )
        return Tuple(arm_conf, sequence)
...
def get_plan_mobile_look_fn(robot, environment=[], max_head_attempts=10, max_base_attempts=100, **kwargs):
    robot_saver = BodySaver(robot, client=robot.client)
    def fn(obj, pose):
        while(True):
            robot_saver.restore()
            ...
            if(robot.head_group is None):
                ...
                num_base_attempts = 0
                for base_conf in sample_visibility_base_confs( robot, obj, pose, client=robot.client ):
                    visible = False
                    base_conf.assign()
                    num_head_attempts = 0
                    while(not visible):
