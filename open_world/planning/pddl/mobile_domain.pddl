(define (domain open-world-tamp)
  ...
  (:action pour
        ...
        :precondition (and (Pour ?a ?to ?p ?from ?g ?aq1 ?aq2 ?at)
                           ...
                           (AtPose ?to ?p) (AtGrasp ?from ?g) (AtConf ?a ?aq1) ; (ArmHolding ?a ?from)
                           ; (not (= ?to ?from))
                      )
  ...
  (:action pick
    ...
    :effect (and (AtGrasp ?o ?g) (CanMove ?a) ; (AtConf ?a ?conf2)
  ...
  (:action place ; TODO: pick and drop action for testing grasp success
    ...
    :effect (and (AtPose ?o ?p) (ArmEmpty ?a) (CanMove ?a)
                 (not (AtGrasp ?o ?g)) (not (Localized ?o))
  ...
  (:action drop
    ...
    :precondition (and (Drop ?a ?o ?g ?b ?bp ?bq ?aq ?at)
                       (AtPose ?b ?bp) (AtGrasp ?o ?g) (AtConf ?a ?aq) (AtConf @base ?bq)
                  )
  ...
  ;                     (AtGrasp ?o ?g) (AtConf ?a ?aq) (AtConf @base ?bq) ; TODO: head conf
  ;                )
  ...
  (:derived (UnsafePregrasp ?o1 ?p1 ?g1) (and (Pose ?o1 ?o1) (Grasp ?o1 ?g1)
    (exists (?o2 ?p2) (and (Pose ?o2 ?p2) (not (= ?o1 ?o2)) (Movable ?o2)
                           (not (CFreePregraspPose ?o1 ?p1 ?g1 ?o2 ?p2))
                           (AtPose ?o2 ?p2)))))
