...
from open_world.planning.primitives import Command, GroupConf, RelativePose, Sequence
...
def get_attribute_facts(objects, color_from_name=COLORS, hue_only=False, **kwargs):
    ...
    for target_name, target_color in color_from_name.items():
        ...
        init.append(
            ("ClosestColor", closest_body, target_name)
        ) 
...
def create_streams(belief, obstacles=[], mobile_base = False, grasp_mode="mesh", verbose=False, **kwargs):
    robot = belief.robot
