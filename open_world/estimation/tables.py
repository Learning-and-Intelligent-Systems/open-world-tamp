...
from open_world.estimation.clustering import cluster_trimesh
...
from open_world.estimation.surfaces import (
    Plane,
    compute_inliers,
    create_surface,
    plane_from_pose,
...
from open_world.simulation.entities import TABLE, UNKNOWN, Label, Table
    ...
    **kwargs
    ...
    print(
        "Relabeled {} points in {:.3f} sec".format(
            num_relabeled, elapsed_time(start_time)
        )
    )
    return seg
    ...
    **kwargs
    ...
    print(
        "Angle error: {:.3f} degrees".format(
            math.degrees((angle_between(plane.normal, Z_AXIS)))
        )
    )
    ...
    if any(side < min_side for side in get_aabb_extent(surface_aabb)[:2]):
        return None
    vertices = get_aabb_vertices(surface_aabb)
...
def cluster_colors(colors, max_clusters=len(COLORS)):
    ...
    kmeans = KMeans(
        n_clusters=max_clusters,  # init='k-means++',
        init=np.array(list(map(remove_alpha, COLORS.values()))),
        n_init=10,
        max_iter=300,
        tol=0.0001,
        verbose=0,
...
def estimate_regions(
    ...
    **kwargs
    ...
    region_colors = [remove_alpha(lp.color) for lp in region_points]
