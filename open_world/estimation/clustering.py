...
import numpy as np
from pybullet_tools.utils import flatten, get_distance, wait_if_gui
from open_world.estimation.observation import LabeledPoint, aggregate_color, draw_points
from open_world.simulation.entities import UNKNOWN, Label
from open_world.simulation.utils import select_indices
...
def relabel_nearby(labeled_points, max_distance=3e-2):
    ...
    for index1, index2 in product(indices, repeat=2):
        if (
            get_distance(labeled_points[index1].point, labeled_points[index2].point)
            < max_distance
        ):
    ...
    for index1 in neighbors_from_vertex:
        ...
        if not candidates:
            continue
        closest_index = min(
            candidates, key=lambda i: get_distance(point, labeled_points[i].point)
        )
...
def cluster_unassigned(labeled_points, groups, **kwargs):
    assigned_indices = set(flatten(groups))
    indices = list(range(len(labeled_points)))
