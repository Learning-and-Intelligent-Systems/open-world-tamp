...
def filter_proximity(times, positions, resolutions):
    ...
    for idx in range(1, len(times) - 1):
        ...
        current_delta = np.absolute(
            get_distance(positions[idx], new_positions[-1]) / resolutions
        )
        next_delta = np.absolute(
            get_distance(positions[idx + 1], new_positions[-1]) / resolutions
        )
    ...
    while True:
        ...
        for sign in [-1, +1]:
            ...
            for i in range(len(position)):
                candidates.extend(
                    t for t in curve.solve(target[i])[i] if t1 < t < end_t
                )
                # curve.roots()
        if not candidates:
            break
