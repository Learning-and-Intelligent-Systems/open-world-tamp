...
class Snowplow(Planner):
        ...
        for attach_pose in attachment_poses:
            ...
            while True:
                ...
                q_place, grasp, obj = self.env.sample_placement(
                    p_attach[-1], obj_obstruction, self.G, p_through_voxels
                )
                if q_place is None:
                    ...
                    break
                p_place = self.a_star(
                    attach_pose, q_place, v_p, attachment=[obj_obstruction, grasp, obj]
                )
                if p_place is None:
                ...
                self.env.movable_boxes.append(obj_obstruction)
                return (
                    [(x, None) for x in p_attach]
                    + [(y, [obj_obstruction, grasp, obj]) for y in p_place]
    ...
    def action_fn(
        self, path, v_0, extended=None, ignore_movable=False, attachment=None
    ):
        ...
        for q_prime_i in self.G.neighbors[self.G.vex2idx[q]]:
            ...
            if not collisions.shape[0] > 0 and (ignore_movable or coll_objects is None):
                if len(path) == 1:
                    v_q = v_0.union(
                        self.env.get_optimistic_vision(q, self.G, attachment=attachment)
                    )
                else:
