...
class RRT(Planner):
    ...
    def get_plan(self, **kwargs):
        ...
        self.movable_handles = self.env.plot_grids(
            visibility=False, occupancy=True, movable=True
        )
    ...
    def execute_path(self, path, ignore_movable=False):
        for qi, q in enumerate(path):
            ...
            for next_qi in path[qi:]:
                if self.env.check_conf_collision(
                    next_qi, ignore_movable=ignore_movable
                ):
    ...
    def rrt(
        ...
        for _ in range(n_iter):
            goal_sample = np.random.choice(
                [True, False], 1, p=[goal_bias, 1 - goal_bias]
            )[0]
            if goal_sample:
                rand_vex = goal
            else:
            ...
            for k in range(2):
                ...
                if self.env.check_collision_in_path(
                    near_vex,
                    new_vex,
                    ignore_movable=ignore_movable,
    ...
    def sample_from_vision(self):
        ...
        free_points = [
            (free[0] * resolution[0], free[1] * resolution[1])
...
def dijkstra(G):
    """Dijkstra algorithm for finding shortest path from start position to
    end."""
