...
LIGHT_GREY = pbu.RGBA(0.7, 0.7, 0.7, 1)
...
CAMERA_WIDTH = 128
CAMERA_HEIGHT = 128
CAMERA_MATRIX = pbu.get_camera_matrix(CAMERA_WIDTH, CAMERA_HEIGHT, fx, fy)
FAR = 3
...
class Environment(ABC):
    ...
    def get_current_q(self, joints):
        return pbu.get_joint_positions(self.robot, joints)
    ...
    def validate_plan(self, plan):
        ...
        for wall in self.room.walls:
            for voxel in self.occupancy_grid.voxels_from_aabb(
                pbu.scale_aabb(pbu.get_aabb(wall, client=self.client), 0.98)
            ):
        ...
        for q, attachment in plan:
            ...
            if (
                len(
                    set(
                        self.occupancy_grid.occupied_voxels_from_aabb(
                            self.aabb_from_q(q)
                        )
                    )
                )
            ...
            if attachment is not None:
                obj_aabb = self.movable_object_oobb_from_q(
                    attachment[0], q, attachment[1]
                ).aabb
                if (
                    len(set(self.occupancy_grid.occupied_voxels_from_aabb(obj_aabb)))
                    > 0
                ):
                    collisions.append((q, attachment))
                    continue
            ...
            for obj in self.room.movable_obstacles:
                if attachment is not None:
                    if attachment[2] == obj:
                        continue
                object_aabb = pbu.scale_aabb(pbu.get_aabb(obj), 0.75)
                if pbu.aabb_overlap(object_aabb, self.aabb_from_q(q)):
                    collisions.append((q, attachment))
                    break
                if attachment is not None:
                    obj_aabb = self.movable_object_oobb_from_q(
                    ...
                    if pbu.aabb_overlap(object_aabb, obj_aabb):
                        collisions.append((q, attachment))
                        break
            ...
            if (
                len(
                    set(
                        self.visibility_grid.occupied_voxels_from_aabb(
                            self.aabb_from_q(q)
                        )
                    )
                )
            ...
            if attachment is not None:
                ...
                if (
                    len(set(self.visibility_grid.occupied_voxels_from_aabb(obj_aabb)))
                    > 0
                ):
                    collisions.append((q, attachment))
                    continue
    ...
    def update_movable_boxes(self, camera_image, ignore_obstacles=[], **kwargs):
        """Given an image from the camera updates all the detected objects that
        ...
        relevant_cloud = [
            ...
            if pbu.aabb_contains_point(lp.point, self.room.aabb)
        ]

        object_points = defaultdict(list)
        for labeled_point in relevant_cloud:
            if (
                labeled_point.label[0] in self.room.movable_obstacles
                and labeled_point.label[0] not in ignore_obstacles
            ):
        ...
        for _, points in object_points.items():
            new_boxes.append(pbu.OOBB(pbu.aabb_from_points(points), pbu.Pose()))
        ...
        def test_in(box, boxes):
            return any(
                [
                    (
                        all(np.array(box.aabb.upper) == np.array(q.aabb.upper))
                        and all(np.array(box.aabb.lower) == np.array(q.aabb.lower))
                    )
                    for q in boxes
                ]
            )
        ...
        for movable_box in self.movable_boxes:
            for new_box in new_boxes:
                if pbu.aabb_overlap(movable_box.aabb, new_box.aabb):
                    all_new_boxes.append(
                        pbu.OOBB(
                            pbu.aabb_union([movable_box.aabb, new_box.aabb]), pbu.Pose()
                        )
                    )
        ...
        while merging:
            ...
            for box1_i in indexes:
                ...
                for box2_i in indexes:
                    ...
                    if box1_i == box2_i:
                        continue
                    if pbu.aabb_overlap(box1.aabb, box2.aabb):
                        all_new_boxes.append(
                            pbu.OOBB(pbu.aabb_union([box1.aabb, box2.aabb]), pbu.Pose())
                        )
                        indexes.add(length)
                        length += 1
                        merging = True
        ...
        for box in final_voxes:
            if pbu.get_aabb_volume(box.aabb) >= 0.01:
        ...
        for movable_box in all_new_boxes:
            for voxel in self.occupancy_grid.voxels_from_aabb(
                pbu.scale_aabb(movable_box.aabb, 1.3)
            ):
    ...
    def update_occupancy(self, q, camera_image, ignore_obstacles=[], **kwargs):
        ...
        occ_aabb = pbu.AABB(
            lower=(
                self.room.aabb[0][0] - 1,
                self.room.aabb[0][1] - 1,
                self.room.aabb[0][2],
            ),
            upper=(
                self.room.aabb[1][0] + 1,
                self.room.aabb[1][1] + 1,
                self.room.aabb[1][2],
            ),
        )
        relevant_cloud = [
            lp
            for lp in iterate_point_cloud(camera_image, **kwargs)
            if pbu.aabb_contains_point(lp.point, occ_aabb)
        ]
    ...
    def lidar_scan(self, q, radius=5, angle=2 * np.pi):
        """Gets the obstruction voxels from a lidar scan at a certain distance
        and angle from the base of the robot.
        ...
        for angle in angles:
            ...
            if not path[1]:
                ...
                for voxel in path[0]:
                    ...
                    vox_aabb = pbu.AABB(
                        lower=vox_aabb[0] + np.ones(3) * 0.01,
                        upper=vox_aabb[1] - np.ones(3) * 0.01,
                    )
                    for elem in self.room.walls + self.objects:
                        obj_aabb = pbu.get_aabb(elem, client=self.client)
                        if pbu.aabb_overlap(obj_aabb, vox_aabb):
                            voxels.add(voxel)
                            flag = True
                            break
        return voxels
    ...
    def setup_visibility_grid(self):
        ...
        surface_aabb = pbu.AABB(
            lower=self.room.aabb.lower + np.array([-1, -1, 0]),
            upper=(
                self.room.aabb.upper[0] + 1,
                self.room.aabb.upper[1] + 1,
                GRID_RESOLUTION,
            ),
        )
    ...
    def get_optimistic_path_vision(self, path, G, attachment=None, obstructions=set()):
        """Gets optimistic vision along a path. Optimistic vision is defined as
        the vision cone from a certain configuration, unless the configuration
        has already been viewed and the actual vision is known.
        ...
        for qi, q in enumerate(path):
            vision.update(
                self.get_optimistic_vision(
                    q, G, attachment=attachment, obstructions=obstructions
                )
            )
            if len(q) == 2:
                attach = path[qi][1]
                ...
                if attach is None and attach_prev is not None and qi > 0:
                    ...
                    vision.update(
                        self.visibility_grid.voxels_from_aabb(
                            pbu.scale_aabb(obj_oobb.aabb, 1.2)
                        )
                    )
        if attachment is not None:
            q = path[-1]
    ...
    def in_view_cone(self, points, path, attachment=None):
        ...
        for q in path:
            ...
            paths_rays.append(
                np.expand_dims(np.all(((s > 0) & (s < CAMERA_HEIGHT)), axis=0), axis=0)
            )
    ...
    def get_optimistic_vision(self, q, G, attachment=None, obstructions=set()):
        """Gets the optimistic vision of a specified configuration. Optimistic
        vision is defined as the vision cone from a certain configuration,
        unless the configuration has already been viewed and the actual vision
        is known.
        ...
        default_vision = np.array(list(default_one)) + (
            np.array([q[0], q[1], GRID_RESOLUTION])
            / np.array(G.res)
            * np.array([1, 1, 0])
        )
        ...
        final_voxels = set()
        pose = pbu.Pose(point=pbu.Point(x=q[0], y=q[1], z=0), euler=[0, 0, q[2]])
        camera_pose = pbu.multiply(pose, self.camera_pose)
        for voxel in resulting_voxels:
            result = self.check_voxel_path_coll(
                voxel,
    ...
    def check_voxel_path_coll(self, start_cell, goal_cell, grid, extra_obs=set()):
        """Check for path between two cells in a grid using Bresenham's
        Algorithm and decide if a cell in the path is occupied, therefore
        colliding.
    ...
    def find_path_movable_obstruction(self, path, attachment=None):
        """Find the first movable object bounding box that this path collides
        with.
        ...
        for q in path:
            ...
            for movable_box in self.movable_boxes:
                if attachment is not None:
                    if pbu.aabb_overlap(movable_box.aabb, attachment[0].aabb):
                        continue

                intersec = pbu.aabb_intersection(aabb, movable_box.aabb)
                if intersec is not None:
                    volume = pbu.get_aabb_volume(intersec)
                    if volume > best_volume:
                        best_volume = volume
                        best_obj = movable_box
                if attachment is not None:
                    intersec = pbu.aabb_intersection(
                        self.movable_object_oobb_from_q(
                            attachment[0], q, attachment[1]
                        ).aabb,
                        movable_box.aabb,
                    )
    ...
    def sample_attachment_push(
        self, aabb, G, obstructions=set(), enforced_obstacles=[]
    ):
        """Helper function to sample an attachment position for the case that
        the object can only be pushed.
        ...
        attachments = [
            [round(mid_point[0], 1), aabb.lower[1], round(np.pi / 2, 3)],
        ...
        for i in range(len(attachments)):
            ...
            if i == 0:
                attachments[i][1] = (
                    attachments[i][1] - (robot_aabb[1][0] + G.res[1]) + G.res[1] / 2
                )
                attachments[i][1] = round(
                    attachments[i][1] - (attachments[i][1] % G.res[1]), 1
                )
            elif i == 1:
                attachments[i][1] = (
                    attachments[i][1] + (robot_aabb[1][0] + G.res[1]) + G.res[1] / 2
                ...
                attachments[i][0] = (
                    attachments[i][0] - (robot_aabb[1][0] + G.res[0]) + G.res[0] / 2
                )
                attachments[i][0] = round(
                    attachments[i][0] - (attachments[i][0] % G.res[0]), 1
                )
            else:
                attachments[i][0] = (
                    attachments[i][0] + (robot_aabb[1][0] + G.res[0]) + G.res[0] / 2
        ...
        for pos in attachments:
            obst, coll = self.obstruction_from_path(
                [tuple(pos)],
                obstructions,
                ignore_movable=True,
    ...
    def sample_attachment_poses(
        self, movable_object, G, radius=1, obstructions=set(), enforced_obstacles=[]
    ):
        """Given an object to move, sample valid poses around it for a
        ...
        for obj in self.objects:
            if pbu.aabb_contains_point(
                pbu.get_aabb_center(movable_object.aabb),
                pbu.get_aabb(obj, client=self.client),
            ):
                break

        return self.sample_attachment_push(
            movable_object.aabb,
            G,
    ...
    def get_movable_box_from_aabb(self, aabb):
        for movable_box in self.movable_boxes:
            if pbu.aabb_overlap(movable_box.aabb, aabb):
                return movable_box
    ...
    def sample_placement(
        ...
        """Samples a placement position for an object such that it does not
        collide with a given path.
        ...
        base_pose = pbu.Pose(
            point=pbu.Point(x=q_start[0], y=q_start[1]),
            euler=pbu.Euler(yaw=q_start[2]),
        )
        obj_pose = pbu.Pose(point=pbu.get_aabb_center(coll_obj.aabb))
        base_grasp = pbu.multiply(pbu.invert(base_pose), obj_pose)
        good = False
        ...
        while not good:
            ...
            if not pbu.aabb_contains_point(
                list(rand_q[:2]) + [0],
                pbu.get_aabb(self.room.floors[0], client=self.client),
            ):
                continue

            aabb = self.movable_object_oobb_from_q(coll_obj, rand_q, base_grasp).aabb
            if pbu.aabb_overlap(aabb, self.aabb_from_q(rand_q)):
                continue
            if in_viewed:
    ...
    def sample_push_placement(
        ...
        """Helper function for finding a placement position for an object that
        can only be pushed.
    ...
    def movable_object_oobb_from_q(
        self, movable_object_oobb, q, grasp, visualize=False
    ):
        ...
        obj_pose = pbu.multiply(robot_pose, grasp)
        aabb_object, _ = pbu.recenter_oobb((movable_object_oobb.aabb, obj_pose))
        object_aabb = pbu.AABB(
            lower=[
                aabb_object[0][0] + obj_pose[0][0],
                aabb_object[0][1] + obj_pose[0][1],
                aabb_object[0][2] + obj_pose[0][2],
            ],
            upper=[
                aabb_object[1][0] + obj_pose[0][0],
                aabb_object[1][1] + obj_pose[0][1],
                aabb_object[1][2] + obj_pose[0][2],
            ],
        )
        if visualize:
            pbu.draw_aabb(robot_aabb)
            pbu.draw_aabb(object_aabb)
        new_object = pbu.OOBB(aabb=object_aabb, pose=pbu.Pose())
        return new_object

    def clear_noise_from_attached(self, q, attachment):
        """Clears any obstructions and visualization noise caused by moving
        ...
        for voxel in self.occupancy_grid.voxels_from_aabb(
            scale_aabb(obj_oobb.aabb, 1.1)
        ):
            self.occupancy_grid.set_free(voxel)

    def move_robot(self, q, joints, attachment=None):
        """Moves the robot model to a desired configuration. If an object is
        attached, move it as well.
        ...
        """
        pbu.set_joint_positions(self.robot, joints, q, client=self.client)
        if attachment is not None:
            robot_pose = pbu.Pose(
                point=pbu.Point(x=q[0], y=q[1]),
                euler=pbu.Euler(yaw=q[2]),
            )
            obj_pose = pbu.multiply(robot_pose, attachment[1])
            pbu.set_pose(attachment[2], obj_pose, client=self.client)
    ...
    def gained_vision_from_conf(self, q):
        """Given a configuration, compute the voxels corresponding to the cone
        of vision.
    ...
    def visibility_voxels_from_path(self, path, attachment=None):
        """Finds the set of voxels that correspond to the swept volume
        traversed by a path in the visibility space.
    ...
    def visibility_points_from_path(self, path, attachment=None):
        """Finds the set of points that correspond to the swept volume
        traversed by a path in the visibility space.
        ...
        for q in path:
            aabb = self.aabb_from_q(q)
            # vis_idx = np.all( (aabb.lower<=vis_points) & (vis_points<=aabb.upper), axis=1 )
            points_from_occ = self.visibility_grid.occupied_voxels_points_from_aabb(
                aabb
            )
            ...
            if visibility_points is None:
                ...
                visibility_points = np.concatenate(
                    [visibility_points, points_from_occ], axis=0
                )
            if attachment is not None:
                aabb = self.movable_object_oobb_from_q(
                    attachment[0], q, attachment[1]
                ).aabb
                # vis_idx = np.all((aabb.lower <= vis_points) & (vis_points <= aabb.upper), axis=1)
                points_from_occ = self.visibility_grid.occupied_voxels_points_from_aabb(
    ...
    def obstruction_from_path(
        ...
        """Finds the set of voxels from the occupied space that a given path
        ...
        """
        occ_points_from_obs = np.array(
            [list(self.occupancy_grid.center_from_voxel(vox)) for vox in obstruction]
        )
        ...
        for q in path:
            ...
            if occupancy_points is None:
                ...
                occupancy_points = np.concatenate(
                    [occupancy_points, points_from_occ], axis=0
                )
            if len(obstruction) > 0:
                vis_idx_from_obs = np.all(
                    (aabb.lower <= occ_points_from_obs)
                    & (occ_points_from_obs <= aabb.upper),
                    axis=1,
                )
                occupancy_points = occupancy_points.reshape(-1, 3)
                occupancy_points = np.concatenate(
                    [occupancy_points, occ_points_from_obs[vis_idx_from_obs]], axis=0
                )
            ...
            if attachment is not None:
                ...
                points_from_occ = self.occupancy_grid.occupied_voxels_points_from_aabb(
                    aabb
                )
                # vis_idx = np.all((aabb.lower <= occ_points) & (occ_points <= aabb.upper), axis=1)
            ...
            for movable_box in enforced_obstacles:
                if pbu.aabb_overlap(movable_box.aabb, aabb):
                    ...
                    break
                if attachment is not None:
                    if pbu.aabb_overlap(movable_box.aabb, obj_aabb):
                        movable_coll = movable_box
                        occupancy_points = np.array(list(q))
                        break
    ...
    def add_chair(self):
        blocking_chair = pbu.load_model(
            "../models/partnet_mobility/179/mobility.urdf", scale=0.5
        )
        chair_color = (0.8, 0.8, 0, 1)
        link_names = pbu.get_link_names(
            blocking_chair, pbu.get_all_links(blocking_chair)
        )
        p.changeVisualShape(
            blocking_chair, link_names.index("link_15"), rgbaColor=chair_color
        )
        p.changeVisualShape(
            blocking_chair, link_names.index("link_15_helper"), rgbaColor=chair_color
        )
        return blocking_chair

    def remove_movable_object(self, movable_obj):
        """Given a movable object, remove it from the record of movable
        objects.
        ...
        for obj in self.movable_boxes:
            if not (
                all(np.array(obj.aabb.upper) == np.array(movable_obj.aabb.upper))
                and all(np.array(obj.aabb.lower) == np.array(movable_obj.aabb.lower))
            ):
                new_l.append(obj)
        self.movable_boxes = new_l
    ...
    def set_defaults(self, robot, **kwargs):
        ...
        joints, values = zip(
            *[
                (pbu.joint_from_name(robot, k, client=self.client), v)
                for k, v in DEFAULT_JOINTS.items()
            ]
        )
        pbu.set_joint_positions(robot, joints, values, client=self.client)

    def setup_robot(self):
        """Setups the robot object for manipulation and visualization in the
        virtual environment."""
        ...
        robot_body = pbu.load_pybullet(
            MOVO_PATH, fixed_base=True, scale=1.15, client=self.client
        )

        self.set_defaults(robot_body)

        self.camera_pose = pbu.get_link_pose(
            robot_body, pbu.link_from_name(robot_body, "kinect2_rgb_optical_frame")
        )

        self.joints = [
            pbu.joint_from_name(robot_body, "x"),
            pbu.joint_from_name(robot_body, "y"),
            pbu.joint_from_name(robot_body, "theta"),
        ]
        return robot_body

    def plot_grids(self, visibility=False, occupancy=False, movable=False):
        """Visualizes the different grids in the simulation based on the
    ...
    def get_robot_vision(self) -> pbu.CameraImage:
        ...
        camera_link = pbu.link_from_name(
            self.robot, "kinect2_rgb_optical_frame", client=self.client
        )
        camera_pose = pbu.get_link_pose(self.robot, camera_link, client=self.client)
    ...
    def display_goal(self, goal, display=False):
        if display:
            GOAL_RADIUS = 0.4
            GOAL_HEIGHT = 4
            goal_pole = pbu.create_cylinder(
                GOAL_RADIUS, GOAL_HEIGHT, color=pbu.RGBA(0, 0.9, 0, 0.4)
            )
            pbu.set_pose(
                goal_pole,
                pbu.Pose(pbu.Point(x=goal[0], y=goal[1], z=-GOAL_HEIGHT / 2.0 + 0.1)),
                client=self.client,
            )
            return goal_pole

    def create_closed_room(
        self, length, width, center=[0, 0], wall_height=2, movable_obstacles=[]
    ):
        ...
        floor = self.create_pillar(width=width, length=length, color=pbu.TAN)
        pbu.set_pose(
            floor, pbu.Pose(pbu.Point(x=center[0], y=center[1])), client=self.client
        )

        wall_thickness = 0.1
        wall_1 = self.create_pillar(
            width=width, length=wall_thickness, height=wall_height, color=LIGHT_GREY
        )
        pbu.set_pose(
            wall_1,
            pbu.Pose(
        ...
        wall_3 = self.create_pillar(
            length=length, width=wall_thickness, height=wall_height, color=LIGHT_GREY
        )
        pbu.set_pose(
            wall_3,
            pbu.Pose(
    ...
    def create_pillar(self, width=0.25, length=0.25, height=1e-3, color=None, **kwargs):
        """Creates a pillar which translates to creating a box in the
        simulation of the specified dimensions.
    ...
    def get_centered_aabb(self):
        """Gets the aabb of the current position of the robot centered around
        the origin.
        ...
        centered_aabb, _ = pbu.recenter_oobb(
            (pbu.get_aabb(self.robot, link=4, client=self.client), pbu.Pose())
        )
    ...
    def get_centered_oobb(self):
        """Gets the oobb of the current position of the robot centered around
        the origin.
        ...
        """
        # TODO: Using the base aabb for simplicity. Change later
        aabb = pbu.get_aabb(self.robot, link=4, client=self.client)
        centered_aabb, pose = pbu.recenter_oobb((aabb, pbu.Pose()))
        return pbu.OOBB(centered_aabb, pose)
    ...
    def aabb_from_q(self, q):
        ...
        """
        aabb = pbu.aabb_from_oobb(self.oobb_from_q(q))
        aabb.upper[2] += 0.5
        return aabb
    ...
    def oobb_from_q(self, q):
        ...
        pose = pbu.Pose(
            point=pbu.Point(x=q[0], y=q[1], z=oobb.pose[0][2]), euler=[0, 0, q[2]]
        )
        return pbu.OOBB(oobb.aabb, pose)
