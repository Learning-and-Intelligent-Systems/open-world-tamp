...
import openai
from pyparsing import OneOrMore, ParseResults, nestedExpr
...
def text_to_goal(command):
    ...
    response = openai.Completion.create(
        model="text-davinci-003", prompt=combined, temperature=0, max_tokens=128
    )

    try:
        response_text = (
            response["choices"][0]["text"].replace("A:", "").replace("\n", "")
        )
